...
class TimeSeriesData:
    ...
    def __init__(self,
        ...
        # 检查日期、数据、标签长度是否一致
        # print("shape", len(dates), len(data), len(labels))
...
class TrainValData:
    ...
    def __init__(self,
        ...
        Notes:
            ...
            如果希望预测未来十天的累计收益，则预测时用到的输入数据为  最近的历史数据  来预测
        ...
        self.__data = _np.empty((len(time_series_list),
                                ...
                                dtype=_np.float32)
        # print('len(time_series_list', len(time_series_list)) 499
        # print('len(self.__distinct_dates', len(self.__distinct_dates)) 1450
        # print('self.__feature_counts', self.__feature_counts) 15
    ...
    def __get_in_memory__(self,
        ...
        # print('data', dates_info)
        train_args = (*train_args, normalize)
        val_args = (*val_args, normalize)
        # 将输入的数据、标签片段转化为单个sample包含history日期长度的历史信息
        (val_x, val_y, val_dates_series, not_nan) = __full_tensor_generation__(*val_args)
        np.save('not_nan.npy', not_nan)
        print('val_x.shape', val_x.shape)
        print('val_y.shape', val_y.shape)
        # print('val_dates_series', val_dates_series)
        ...
        (train_x, train_y, train_dates_series, not_nan) = __full_tensor_generation__(*train_args)
    ...
    def __get_period_info__(self, start_date, order="by_date",
        ...
        # print('train_index=', train_start_index, train_end_index)
        # 验证集开始位置(data的开始位置)
        ...
        val_end_index = (train_end_index +
                         validate_length +
                         self.__train_val_gap)
        # print('val_index=', val_start_index, val_end_index)
...
def __full_tensor_generation__(data,
    ...
    return data_all, label_all, dates_series_all, not_nan
