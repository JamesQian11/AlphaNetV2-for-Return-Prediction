...
def get_rolling_training_set(train_length, validate_length):
    stock_data_list = np.load('data/lugutong_training_data_combined.npy', allow_pickle=True).tolist()
    # put stock list into TrainValData() class, specify dataset lengths
    train_val_data = TrainValData(time_series_list=stock_data_list,
...
def get_rolling_testing_set(train_length, validate_length):
    stock_data_list = np.load('data/lugutong_testing_data_combined.npy', allow_pickle=True).tolist()
    # put stock list into TrainValData() class, specify dataset lengths
    train_val_data = TrainValData(time_series_list=stock_data_list,
                                  train_length=train_length,  # 1200 trading days for training 1197
                                  validate_length=validate_length,  # 150 trading days for validation  233
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        for j in range(0, 1):
            ...
            history = model.fit(train_0.batch(500).cache(), validation_data=val_0.batch(500).cache(), epochs=5, callbacks=[tensorboard_callback])
            model.save_weights('model/AlphaNetV2_Weights_zz500_returns_trade_%d_%d' % (i, j))
            # from tensorboardX import SummaryWriter
            # writer = SummaryWriter('runs/scalar_example')
            # for i in range(1):
            #     writer.add_graph(model(), input_to_model=train_0.batch(500).cache(), verbose=False, )
            #     writer.close()
            model.load_weights('model/AlphaNetV2_Weights_zz500_returns_trade_%d_%d' % (i, j))
